...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const url = `${baseUrl}/${locale}/cookie-policy`;

    return {
        title: t('title'),
        description: t('description'),
        keywords: isKo
            ? '쿠키 정책, 쿠키, 개인정보, Clock-Tani'
            : 'cookie policy, cookies, privacy, Clock-Tani',
        alternates: {
            canonical: url,
            languages: { 'x-default': `${baseUrl}/ko/cookie-policy`, 'ko': `${baseUrl}/ko/cookie-policy`, 'en': `${baseUrl}/en/cookie-policy` },
        },
        openGraph: {
            title: t('title'),
            description: t('description'),
...
export default async function CookiePolicyPage(props: { params: Promise<{ locale: string }> }) {
    ...
    const sectionKeys = ['whatAreCookies', 'typesUsed', 'essential', 'analytics', 'advertising', 'manageCookies', 'thirdParty', 'changes', 'contact'] as const;
    const listSections = ['essential', 'analytics', 'advertising', 'manageCookies'];
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px', lineHeight: 1.8 }}>
            ...
            {sectionKeys.map(key => (
                <section key={key} style={{ marginBottom: 28 }}>
                    ...
                    {key === 'contact' && (
                        <p style={{ marginTop: 8 }}>
                            <a href="mailto:admin@teck-tani.com" style={{ color: '#0891b2' }}>
                                admin@teck-tani.com
                            </a>
                        </p>
                    )}
