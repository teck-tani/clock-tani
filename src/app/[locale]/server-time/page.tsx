...
function generateBreadcrumbSchema(locale: string) {
    ...
    return {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 2, "name": isKo ? "서버시간" : "Server Time", "item": `${baseUrl}/${locale}/server-time` }
        ]
    };
}
...
// ===== Page Component =====
const uniqueContentKeys = ['ntp', 'clockDrift'] as const;
...
export default async function ServerTimePage(props: { params: Promise<{ locale: string }> }) {
    ...
    return (
        <>
            ...
            <RelatedGuides currentToolHref="/server-time" />
        </>
    );
}
