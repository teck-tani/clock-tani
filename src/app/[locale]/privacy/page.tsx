...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    return {
        ...
        keywords: isKo
            ? '개인정보처리방침, 개인정보, Clock-Tani, 쿠키 정책'
...
export default async function PrivacyPage(props: { params: Promise<{ locale: string }> }) {
    ...
    const sectionKeys = ['collect', 'purpose', 'thirdParty', 'cookies', 'retention', 'rights', 'changes', 'contact'] as const;
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '40px 20px', lineHeight: 1.8 }}>
            ...
            {sectionKeys.map(key => (
                <section key={key} style={{ marginBottom: 28 }}>
                    ...
                    {['collect', 'purpose', 'thirdParty'].includes(key) && (
